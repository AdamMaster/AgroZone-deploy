...
p = pathlib.Path("client/src/components/layout/container.tsx")
s = p.read_text(encoding="utf-8")

old = """export const Container = ({ children, className }: PropsWithChildren<ContainerProps>) => {
  return <div className={cn('mx-auto max-w-344 px-4 md:px-5', className)}>{children}</div>
}"""

new = """// max-w-360 = 360 * 0.25rem (см. --spacing в globals.css) = 90rem = 1440px —
