...
// Раздел "Услуги" (см. F18 в ROADMAP.md) — это категории самих УСЛУГ, а не
// товаров, и обычный промпт ниже под них плохо подходит: просили "товары,
// культуры, породы, сорта" — и GigaChat честно перечислял, ЧТО именно
// перевозят/обрабатывают ("перевозка картофеля, свеклы, моркови..."), но ни
// разу не упоминал синонимы самого действия ("доставка", "транспортировка",
// "логистика"). Нашли на живом тесте 08.09.2026: запрос "доставка урожая" не
// находил "Грузоперевозки", хотя по смыслу это ровно то, что нужно — просто
// слова "доставка" и "урожай" в терминах категории не было вообще, только
// частные овощи. root ancestor "Услуги" — самый надёжный признак (сама
// категория "Услуги" — корень дерева, ancestorNames для неё пуст, поэтому
// проверяем и её саму, и первого предка в цепочке (ancestorNames[0]).
function isServiceCategory(categoryName: string, ancestorNames: string[]): boolean {
  return categoryName === 'Услуги' || ancestorNames[0] === 'Услуги'
}
...
function buildPrompt(categoryName: string, ancestorNames: string[]): string {
  ...
  if (isServiceCategory(categoryName, ancestorNames)) {
    return (
      `Ты помощник интернет-магазина сельскохозяйственных товаров "AgroZone". ` +
      `Есть категория объявлений об УСЛУГЕ (не товаре): "${categoryName}"${parentContext}. ` +
      `Перечисли через запятую 15-25 слов и словосочетаний, которые человек может ввести в поиск, ` +
      `имея в виду именно эту услугу — обязательно включи разговорные синонимы самого действия/услуги ` +
      `(например для перевозок: "доставка", "транспортировка", "логистика", "экспедирование"), ` +
      `а также конкретные объекты, товары или культуры, к которым эта услуга применяется. ` +
