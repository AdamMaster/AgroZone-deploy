  ...
  return (
    <div className='py-6 text-mist-50'>
      ...
      <div className='flex flex-col gap-2'>
        {reports.map(report => (
          <div key={report.id} className='flex items-start gap-4 rounded-md bg-mist-600/50 p-3'>
            <div className='min-w-0 flex-1'>
              <div className='mb-1 flex items-center gap-2'>
                <Link href={`/ads/${report.ad.id}`} target='_blank' className='hover:text-primary font-medium'>
                  {report.ad.title}
                </Link>
                <span className='rounded-full bg-amber-200 px-1.5 text-[11px] text-mist-950'>
                  {STATUS_LABELS[report.status]}
                </span>
              </div>
              <p className='text-sm text-red-400'>{AD_REPORT_REASON_LABELS[report.reason]}</p>
              {report.comment && <p className='mt-1 rounded-sm bg-mist-600/60 p-2 text-sm'>{report.comment}</p>}
              <p className='mt-4 text-xs text-mist-400'>От: {report.user.displayName ?? 'Пользователь'}</p>
            </div>
