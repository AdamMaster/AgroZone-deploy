...
// Разбирает то, что продавец напечатал, в массив строк для сохранения.
// Разделителем считается и запятая, и пробел (в любом сочетании и любом
// количестве подряд) — обсуждали с пользователем: печатать через пробел
// быстрее (не нужно тянуться к запятой, особенно с телефона), а через
// запятую привычнее и однозначнее читается как список, а не диапазон;
// решили поддержать оба варианта сразу, а не выбирать один. То есть
// "45+, 55, 65", "45+ 55 65" и "45+,55 65" разбираются одинаково.
// Дальше — отбрасывает пустые куски и всё, что не выглядит как "число" или
// "число+" (защита от мусора, который мог проскочить через фильтр ввода при
// вставке текста мышью/со сканера штрих-кодов и т.п.), убирает дубликаты,
